...
Container.prototype.factory = function (id, fn, dependencies, sid) {
	dependencies = dependencies || fn['@require'] || [];
	debug('register factory %s %s', id, dependencies);
	this._register(new Factory(id, dependencies, fn), sid);
}

Container.prototype.singleton = function (id, fn, dependencies, sid) {
	dependencies = dependencies || fn['@require'] || [];
	debug('register singleton %s %s', id, dependencies);
	this._register(new Singleton(id, dependencies, fn), sid);
}

Container.prototype.constructor = function (id, ctor, dependencies, sid) {
	dependencies = dependencies || ctor['@require'] || [];
	debug('register constructor %s %s', id, dependencies);
	this._register(new Constructor(id, dependencies, ctor), sid);
}

Container.prototype.literal = function (id, obj, sid) {
	debug('register literal %s', id);
	this._register(new Literal(id, [], obj), sid);
}

Container.prototype._register = function (comp, sid) {
	// TODO: Pass sid to constructor (??)
	comp._sid = sid;
	this._o[comp.id] = comp;
}

Container.prototype.register = function (id, module, dependencies, mode) {
	var explicitDeps = true;
	...
	mode = mode || this.getModuleType(module) || "@singleton";

	if (mode == "@singleton") {
		if (explicitDeps) {
			return this.singleton(id, module, dependencies);
		} else {
			return this.singleton(id, module);
		}
	} else if (mode == "@factory") {
		if (explicitDeps) {
			return this.factory(id, module, dependencies);
		} else {
			return this.factory(id, module);
		}
	} else if (mode == "@constructor") {
		if (explicitDeps) {
			return this.constructor(id, module, dependencies);
		} else {
			return this.constructor(id, module);
		}
	} else if (mode == "@literal") {
		if (explicitDeps) {
			return this.literal(id, module, dependencies);
		} else {
			return this.literal(id, module);
		}
	} else {
...
Container.prototype._registerModule = function (id, mod, sid) {
	...
	var pattern = this.getModuleType(mod) || "@singleton";

	switch (pattern) {
		case '@factory':
			this.factory(id, mod, dependencies, sid);
			break;
		case '@singleton':
			this.singleton(id, mod, dependencies, sid);
			break;
		case '@constructor':
			this.constructor(id, mod, dependencies, sid);
			break;
		case '@literal':
