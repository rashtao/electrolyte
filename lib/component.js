...
Component.prototype.create = function (container, prefixes) {
    if (this.type != "setterInjection" && prefixes.indexOf(this.id) >= 0) {
        throw new Errors.RecursiveDependencyFound(this.id, prefixes.join(" --> "));
    }
    ...
    if (this.type == "setterInjection" && !this.loaded) {
        ...
        this.createGetters(instance, container, prefixes.slice());
        if(typeof instance._ic_init === "function"){
            instance._ic_init();
        }
    } else {
        instance = this.instantiate.apply(this, args);
